...
The `flights` data frame has `r nrow(flights)` rows. Each row represents a **flight**.
...
-   Question: What does it mean for the `dep_delay` to have a negative value?
    ...
    The flight was not delayed, and left early.
...
The plane tail number w/ the longest departure delay is **N384HA**.
...
This is filter by flights that have destinations RDU and GSO, either with an arrival delay less than zero OR a departure delay less than 0, i.e. this is looking at early arrival or early flights. 
...
flights %>%
  ...
  slice_max(n, n=1)
```
November 27, with 1014 flights that day.
...
flights %>%
  ...
  group_by(origin) %>%
  summarize(prop_rdu = mean(rdu_bound))
```
...
```
LGA has the shortest departure delay.
...
flights %>%
  ...
  slice(c(slice_head(order_by = mean_dist, n = 5),
          slice_tail(order_by = mean_dist, n = 5)))
```
